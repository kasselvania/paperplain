...
export function validateRequestSecret(secret) {
  if (typeof secret !== "string" || !REQUEST_SECRET_PATTERN.test(secret)) {
    throw new Error(
      "PAPERPLAIN_REQUEST_SECRET must be exactly 64 lowercase hexadecimal characters (32 random bytes) with no quotes, spaces, or line breaks.",
    );
  }
...
function requestSecretKey(secret) {
  return Buffer.from(validateRequestSecret(secret), "hex");
}
...
export function signHostedRequest({
  ...
  const signature = createHmac("sha256", requestSecretKey(secret))
    .update(canonical)
    .digest("hex");
